...
const ProfilePage = () => {
  ...
  const handleUpdateEvent = async (eventId, updatedData) => {
    try {
      const res = await axios.put(`http://localhost:5000/api/update/:id/${eventId}`, updatedData);
  ...
  const handleDeleteEvent = async (eventId) => {
    try {
      await axios.delete(`http://localhost:5000/api/cancel/:id/${eventId}`);
      setData(prev => ({
        ...prev,
        ...
        <Container className="d-flex justify-content-center" style={{ width: "49%" }}>
          <Card className="text-white p-4 rounded-4" style={{ width: "400px", backgroundColor: "rgba(180, 180, 180, 0.4)" }}>
            <div className="d-flex flex-column align-items-center">
              ...
              >
                <img
                  src={data?.user?.profileImage?.imageURL || defaultUserPic}
